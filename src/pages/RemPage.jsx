...
export function RemPage() {
  ...
  return (
    <AppShell title="Gestion REM - Nivel de Cuidado" actions={<div className="filters"><select value={anio} onChange={(e) => setAnio(e.target.value)}><option value="">Todos</option>{years.map((value) => <option key={value} value={value}>{value}</option>)}</select><select value={mes} onChange={(e) => setMes(e.target.value)}><option value="">Todos los meses</option>{MONTH_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select value={nivelCodigo} onChange={(e) => setNivelCodigo(e.target.value)}>{data.niveles.map((item) => <option key={item.codigo} value={item.codigo}>{item.nombre}</option>)}</select><select value={glosa} onChange={(e) => setGlosa(e.target.value)}><option value="">Todas las glosas</option>{data.glosas_base.map((item) => <option key={item.clave} value={item.clave}>{item.titulo}</option>)}</select></div>}>
