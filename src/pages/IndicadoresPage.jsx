...
export function IndicadoresPage() {
  ...
  if (!total || !metrics) return <AppShell title="Indicadores Clínicos">Cargando...</AppShell>;

  const labels = total.egresos.map((item) => monthLabel(item.mes));

  return (
    <AppShell title="Indicadores Clínicos">
      <section className="kpis">
        <div className="indicator-box success"><span>TASA OCUPACIÓN GLOBAL</span><h2>{metrics.ocupacion.toFixed(1)}%</h2></div>
        <div className="indicator-box primary"><span>PROMEDIO DE ESTADÍA</span><h2>{metrics.promedioEstada.toFixed(1)} días</h2></div>
        <div className="indicator-box warning"><span>ÍNDICE DE ROTACIÓN</span><h2>{metrics.rotacion.toFixed(1)}</h2></div>
        <div className="indicator-box danger"><span>TASA LETALIDAD</span><h2>{metrics.letalidad.toFixed(1)}%</h2></div>
      </section>
      <section className="grid-2">
        <div className="card"><h3>Ocupación mensual</h3><div className="chart-container"><ChartCanvas type="line" data={{ labels, datasets: [{ label: "Ocupación %", data: total.egresos.map((item, index) => metrics.ocupacion + Math.sin(index) * 5), borderColor: "#2563eb", backgroundColor: "rgba(37,99,235,0.25)", fill: true, tension: 0.4 }] }} options={{ responsive: true, maintainAspectRatio: false }} /></div></div>
        <div className="card"><h3>Evolución de egresos</h3><div className="chart-container"><ChartCanvas type="bar" data={{ labels, datasets: [{ label: "Altas", data: total.egresos.map((item) => item.altas), backgroundColor: "#10b981" }, { label: "Fallecidos", data: total.egresos.map((item) => item.fallecidos), backgroundColor: "#ef4444" }] }} options={{ responsive: true, maintainAspectRatio: false }} /></div></div>
      </section>
    </AppShell>
  );
}
